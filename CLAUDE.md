...
- **Chrome:** `components/chrome/SiteHeader.tsx` (client; mega menü, arama overlay `⌘K`, mobil drawer, sticky), `SiteFooter.tsx`. Nav verisi `lib/nav.ts` (Strapi'den, fallback'li). `layout.tsx` bunları kullanır. Mega menüde (Araçlar + İçerik) sağdaki "öne çıkan" önizleme artık **hover/focus'ta dinamik**: üzerine gelinen tool/yazının kapak görseli + kategori chip'i + başlık + açıklaması gelir (`NavItem.imageUrl/catLabel/desc`). Tool kapak görseli için Strapi `Tool`'a `kapakGorseli` media alanı eklendi (blog'da zaten vardı); api-client `TOOL_POPULATE` + `listTools` bunu populate eder. **CMS redeploy + admin'den her tool'a görsel yüklenmesi gerekir** yoksa placeholder gösterilir.
...
- **ÖNCELİK — `stok-devir-hizi.ts` hesaplayıcısı.** SEMrush (tr): `stok devir hızı hesaplama` **720 aylık arama / KD 21** = sitedeki en yüksek hacimli tek fırsat (EOQ 10, emniyet stoğu 20'nin çok üstünde). Tool mantığı + registry + `/araclar/stok-devir-hizi` + blog çifti. Formül: SMM / ortalama stok; "yorumlama" ve "ideal oran (4-6)" intent'i de aranıyor, içeriğe gir. Rakip: HarmonyERP, Vepara, Mikro (hepsi e-ticaret/ERP açısı; mühendislik açısı + ortalama stok varyantları farklılaştırıcı).
